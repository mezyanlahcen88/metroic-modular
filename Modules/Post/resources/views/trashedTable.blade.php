            ...
            <tr class="text-center">
                <td>
                    <div class="symbol symbol-40px symbol-circle">
                        <img src="{{ URL::asset(getPicture($object->avatar, 'posts')) }}" alt="image">
                    </div>
                </td>
                @foreach ($tableRows as $key => $value)
                    <td> {{ $object->$key }}</td>
                ...
                <td>
                    @include('components.softDeleteActions',[
                        'restoreRoute'=>'post.restore',
                        'forceDeleteRoute'=>'post.forceDelete',
                        'restorePermission'=>'post-restore',
                        'forceDeletePermission'=>'post-forse-delete',
                    ])
                </td>
            </tr>
        @endforeach
